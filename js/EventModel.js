define(
	function() {
		...
		EventModel.prototype._on = function(type,callback) {
		...
		EventModel.prototype._subscribe = function(type,callback,args) {
			var handler = this._on(type,callback);
			handler.fire(args);
			return handler;
		}
		
		EventModel.prototype._fire = function(type,args) {
		...
		EventModel.prototype.__getHandlers = function(type) {
			if(!this._handlers[type])
				this._handlers[type] = [];
		...
		EventModel.prototype.__getHandlersCopy = function(type) {
			return [].concat(this.__getHandlers(type));
		}

		EventModel.prototype.__addHandler = function(type,handler) {
			var handlers = this.__getHandlers(type);
			handlers.push(handler);
		}

		EventModel.prototype.__removeHandler = function(type,handler) {
			var handlers = this.__getHandlers(type);
		...
		EventModel.prototype.__removeHandlerType = function(type) {
